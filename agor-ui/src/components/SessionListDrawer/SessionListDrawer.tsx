...
import { Drawer, Select, Input, List, Typography, Space, Badge, theme } from 'antd';
import { SearchOutlined, RobotOutlined } from '@ant-design/icons';
import { Session, Board } from '../../types';
...
export const SessionListDrawer: React.FC<SessionListDrawerProps> = ({
  ...
  const filteredSessions = boardSessions.filter((session) =>
  ...
  return (
    ...
    >
      ...
      <div style={{
        ...
        <Title level={5} style={{ marginBottom: 8 }}>Board</Title>
        <Select
          style={{ width: '100%' }}
                ...
                description={
                  <Space direction="vertical" size={2} style={{ width: '100%' }}>
                    <Text type="secondary" style={{ fontSize: 12 }}>
                      {session.agent} • {session.tasks.length} {session.tasks.length === 1 ? 'task' : 'tasks'}
                    </Text>
                    <Text type="secondary" style={{ fontSize: 12 }}>
